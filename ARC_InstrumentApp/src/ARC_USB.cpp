...
public:
		...
		m_mutex(const_cast<std::timed_mutex&>(mutex))
	{
		...
		m_isLocked = true; /* m_mutex.try_lock_for(Ms(TimeOut));*/
	}
	~timed_lock_guard() {
//		if (m_isLocked)
//			m_mutex.unlock();
...
ARC_USB::ARC_USB(const char* portName, int MonoSerial) :
	asynPortDriver(portName, 1, 
		asynDrvUserMask | asynInt32Mask | asynFloat64Mask | asynOctetMask | asynEnumMask,
		asynDrvUserMask | asynInt32Mask | asynFloat64Mask | asynOctetMask | asynEnumMask,
...
/*	asynPortDriver(const char *portName, int maxAddr, int interfaceMask, int interruptMask,
	...
	asynStatus status = asynSuccess;
	status = createParam("WAVELENGTH", asynParamFloat64, &m_WavelengthPV);
...
asynStatus ARC_USB::readInt32(asynUser *pasynUser, epicsInt32 *value)
{
	...
	if ((status != asynSuccess) && (m_slitWidth))
		status = m_slitWidth->readInt32(pasynUser, value);
...
asynStatus ARC_USB::writeInt32(asynUser *pasynUser, epicsInt32 value)
{
	...
	if ((status != asynSuccess) && (m_slitWidth))
		status = m_slitWidth->writeInt32(pasynUser, value);
	if (status != asynSuccess)
